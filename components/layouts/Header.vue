<template>
  <header id="header-layout">
    <div class="w-full grid grid-cols-[15%,55%,30%] bg-">
      <div
        class="px-5 border-r-[1.5px] border-b-[1.5px] border-solid border-black"
      >
        <img
          src="/images/E-3-name.png"
          height="85"
          class="select-none"
          alt="Al-Musnam Logo 2000x2000"
        />
      </div>
      <nav
        class="w-full border-r-[1.5px] border-b-[1.5px] border-solid border-black inline-flex items-center justify-evenly list-none"
      >
        <li class="text-xl text-black font-normal">About Us</li>
        <li class="text-xl text-black font-normal">Projects</li>
        <li class="text-xl text-black font-normal">Contact</li>
      </nav>
      <div
        class="p-5 border-r-[1.5px] border-b-[1.5px] border-solid border-black items-center"
      >
        <h2 class="text-lg font-bold text-black">coming soon...</h2>
      </div>
    </div>
  </header>
</template>

<script setup></script>

<style></style>
